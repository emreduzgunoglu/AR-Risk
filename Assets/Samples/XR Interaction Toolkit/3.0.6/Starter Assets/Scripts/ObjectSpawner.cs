...
namespace UnityEngine.XR.Interaction.Toolkit.Samples.StarterAssets
{
	...
	public class ObjectSpawner : MonoBehaviour
	{
		...
		public bool TrySpawnObject(Vector3 spawnPoint, Vector3 spawnNormal)
		{
			...
			{
				Debug.LogError($"Invalid spawn index: {objectIndex}. Ensure it's within the range of the prefab list.");
				return false;
			}
			...
			// WorldMap Variant için özel dönüþüm ayarý
			if (newObject.name.Contains("world"))
			{
				newObject.transform.rotation = Quaternion.Euler(-90, 90, 0); // Her zaman yatay olacak þekilde ayarla
				newObject.transform.position += new Vector3(0, 0.1f, 0); // Yerden 0.1 birim yukarý kaldýr
			}
			else
			{
